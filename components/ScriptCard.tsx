    ...
    <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-2xl mb-8">
      <div className="flex flex-col md:flex-row justify-between items-start mb-6 border-b border-slate-700 pb-4 gap-4">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <button onClick={() => copy(JSON.stringify(script, null, 2), 'JSON')} className="flex-1 bg-slate-700 hover:bg-slate-600 px-3 py-2 rounded text-xs flex items-center justify-center gap-2">
            {copiedSection === 'JSON' ? <Check size={14}/> : <FileText size={14}/>} JSON
          </button>
          <button onClick={() => copy(script.caption_seo, 'SEO')} className="flex-1 bg-green-900/30 hover:bg-green-900/50 text-green-400 border border-green-800 px-3 py-2 rounded text-xs flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-6">
          <div className="bg-slate-900/50 p-4 rounded-lg border-l-4 border-purple-500">
            <h3 className="text-purple-400 text-xs font-bold uppercase mb-2">01. O Gancho</h3>
            <p className="text-sm text-slate-300 mb-2"><strong>Visual:</strong> {script.hook_visual_desc}</p>
            <button onClick={handleGenerateImage} disabled={loadingImg} className="bg-purple-600 hover:bg-purple-500 text-white px-3 py-2 rounded text-xs font-bold flex items-center gap-2">
            ...
            <table className="w-full text-xs text-left">
              <thead className="text-slate-500 border-b border-slate-800">
                <tr>
                  ...
                  <th className="py-2 pr-2 flex items-center gap-1"><Monitor size={12}/> Visual</th>
                  <th className="py-2 flex items-center gap-1"><Mic size={12}/> Áudio</th>
                </tr>
              </thead>
              <tbody>
        ...
        <div className="space-y-6">
          <div className="aspect-[9/16] bg-slate-900 rounded-lg border border-slate-700 overflow-hidden flex items-center justify-center">
            {script.generated_image_url ? (
              <img src={script.generated_image_url} className="w-full h-full object-cover" />
            ) : (
              <div className="text-slate-600 text-center p-8">
                <ImageIcon size={48} className="mx-auto mb-2 opacity-20"/>
                <p className="text-xs italic">Frame do Gancho ainda não gerado</p>
              </div>
            )}
          </div>
        </div>
